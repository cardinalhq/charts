...
{{/*
Strict bool reader for mode-critical toggles (ha.enabled). Returns
"true" (string) when the value is the
...
The strict-bool narrow on ha.enabled runs UNCONDITIONALLY — even in POC
mode — so a stringly `--set-string ha.enabled=true` fails with a clear
message regardless of whether the helper is otherwise reached during
this particular template's render path.
*/}}
